...
public class GameFlow {
    ...
    private Counter numberOfLevelNum;
    ...
    public GameFlow(int width, int hight, HighScoresTable table, GUI gui, KeyboardSensor ks, AnimationRunner ar) {
        ...
        this.numberOfLevelNum = new Counter(1);
        this.ar = ar;
        this.win = false;
        this.lose = false;
        this.table = table;
    ...
    public void runLevels(List<LevelInformation> levels) {
        ...
        int numOfLevel = 1;
        double speedOfAilens = 2;
        while (true) {
            GameLevel level = new GameLevel(levelInfo, this.gui, this.ks, this.score,
                    this.numberOfLives, this.ar,
                    speedOfAilens);
            level.initialize();
            ScoreIndicator scorePrint = new ScoreIndicator(score);
            scorePrint.addToGame(level);
            LivesIndicator livesPrint = new LivesIndicator(numberOfLives);
            livesPrint.addToGame(level);
            LevelName levelPrint = new LevelName(levelInfo.levelName() + numOfLevel);
            levelPrint.addToGame(level);
